...
class Alliances(commands.Cog):
    ...
    async def cog_load(self):
        print(
            "🤝 Alliances iniciado correctamente."
        )
    ...
    # ========================================================
    # IMPORTANTE:
    # Este comando NO requiere Gestionar servidor.
    # Cualquier miembro puede utilizarlo.
    # ========================================================
    @app_commands.command(
        name="alianza",
    ...
    async def send_alliance_request(
        ...
        requests_channel_id = config.get(
            "requests_channel_id"
        )
    ...
    async def accept_request(
        ...
        if public_channel:
            ...
            try:
                ...
                print(
                    f"⚠️ Error publicando alianza: {error}"
                )
        else:
            print(
                f"⚠️ No existe canal de publicaciones "
                f"configurado en {guild.name}."
            )
        # ====================================================
        # EDITAR SOLICITUD
        # ====================================================
        try:
            ...
            print(
                f"⚠️ No pude editar solicitud: {error}"
            )
        await interaction.response.send_message(
