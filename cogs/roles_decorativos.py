...
class CrearRolModal(discord.ui.Modal, title="🎨 Crear rol decorativo"):
    ...
    emoji_icono = discord.ui.TextInput(
        label="Emoji para el ícono (Opcional)",
        placeholder="Ej: 💜, 👑, ⭐",
        max_length=5,
        required=False
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        emoji_val = self.emoji_icono.value.strip() or None
        ...
        try:
            role = await guild.create_role(
                name=nombre_val,
                color=color_val,
                display_icon=emoji_val,
                permissions=discord.Permissions.none(),
                reason=f"Rol decorativo creado por {interaction.user}"
            )
        except discord.InvalidArgument:
            return await interaction.response.send_message(
                "❌ El emoji ingresado no es válido. Usa un emoji Unicode estándar (ej: 💜, ⭐).",
                ephemeral=True
            )
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"**Ícono:** {emoji_val if emoji_val else 'Sin ícono'}"
            ),
            color=color_val
        )
