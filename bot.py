...
class MiBot(commands.Bot):
    ...
    async def sync_commands(self):
        ...
        if len(all_commands) > MAX_SLASH_COMMANDS:
            ...
            print(
                f"⚠️ Hay {len(all_commands)} "
                f"comandos slash."
            )

            print(
                "⚠️ Discord permite máximo 100."
            )

            print(
                "⚠️ Se conservarán los primeros "
                "100 comandos."
            )


            self.slash_commands_skipped = (
                all_commands[MAX_SLASH_COMMANDS:]
            )
        ...
        try:
            ...
            print(
                f"⚠️ Error limpiando servidor: "
                f"{type(error).__name__}: {error}"
            )


        # ----------------------------------------------------
        # AGREGAR COMANDOS AL GUILD
        # ----------------------------------------------------
        ...
        if self.slash_commands_skipped:

            print("")

            print(
                f"⚠️ {len(self.slash_commands_skipped)} "
                f"comandos no fueron registrados "
                f"como slash."
            )

            print(
                "➡️ Esos comandos siguen "
                "disponibles mediante "
                f"{PREFIX} si son comandos prefix."
            )
