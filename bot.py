...
class MiBot(commands.Bot):
    ...
    async def load_all_cogs(self):
        ...
        print("=" * 70)
        print(
            f"📦 CARGADOS: {len(self.cogs_loaded)}"
        )
        print(
            f"❌ CON ERROR: {len(self.cogs_failed)}"
        )
        print("=" * 70)
    ...
    def show_commands(self):
        ...
        print(
            f"⌨️ PREFIX: {len(prefix_commands)}"
        )
    ...
    async def sync_commands(self):
        ...
        try:
            ...
            # IMPORTANTE:
            #
            # NO HACEMOS tree.sync() GLOBAL.
            #
            # Todo se registra directamente en el servidor.
            # ------------------------------------------------
        ...
        if TEST_GUILD_ID:
            ...
            try:
                ...
                test_synced = await self.tree.sync(
                    guild=test_guild
                )
                ...
                print(
                    f"🧪 TEST GUILD: "
                    f"{len(test_synced)} comandos"
                )

            except Exception as error:

                print(
                    f"⚠️ TEST GUILD ERROR: "
                    f"{type(error).__name__}: {error}"
                )
...
async def on_connect():
    ...
    print(
        "🟡 Conectado al Gateway de Discord."
    )
