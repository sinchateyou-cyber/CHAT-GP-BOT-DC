...
# COMMAND TREE
# ============================================================
class MiCommandTree(app_commands.CommandTree):
    """
    CommandTree compatible con discord.py 2.6.3.
    IMPORTANTE:
    No usamos un límite artificial de 100 comandos.
    Los comandos se registran normalmente y luego se
    sincronizan en el servidor principal.
    También manejamos correctamente el sentinel MISSING
    que utiliza discord.py cuando guilds no fue proporcionado.
    """
    ...
    def add_command(
        ...
        *,
        ...
        # IMPORTANTE:
        #
        # discord.py puede enviar MISSING cuando guilds
        ...
        if guild is missing:
            guild = None
        # ----------------------------------------------------
        # Si por alguna razón llegan ambos argumentos,
        # los normalizamos.
        # ----------------------------------------------------
        if guild is not None and guilds is not None:
            try:
                normalized_guilds = list(guilds)
            except TypeError:
                normalized_guilds = []
            if guild not in normalized_guilds:
                normalized_guilds.insert(
                    0,
                    guild,
                )
            guild = None
...
class MiBot(commands.Bot):
    ...
    async def sync_commands(self):
        ...
        try:
            ...
            print(
                "🧹 Comandos anteriores del servidor limpiados."
            )
            # ------------------------------------------------
            ...
            print("")
            print(
                f"✅ {self.sync_count} SLASH COMMANDS "
                f"SINCRONIZADOS"
            )
            print(
                f"🏠 SERVIDOR: {GUILD_ID}"
            ...
            if self.cogs_failed:
                ...
                for extension, error in self.cogs_failed:
                    print(
                        f"   ❌ {extension}"
                    )
                    print(
...
async def on_ready():
    ...
    if bot.cogs_failed:
        print("")
        print(
            "⚠️ HAY COGS CON ERRORES:"
        )
        for extension, error in bot.cogs_failed:
            print(
                f"   ❌ {extension}: "
                f"{type(error).__name__}"
            )
        ...
        print("")
        print(
            "✅ TODOS LOS COGS SE CARGARON CORRECTAMENTE"
        )
    print("=" * 70)
# ============================================================
# ERROR PREFIX
